...
async fn create(db: Db, post: Json<Post>) -> Result<Created<Json<Post>>> {
    let item = post.clone();
    db.run(move |conn| {
        conn.execute("INSERT INTO faucets (account, amount) VALUES (?1, ?2)",
            params![item.account, item.amount])
    }).await?;
...
async fn list(db: Db) -> Result<Json<Vec<i64>>> {
    let ids = db.run(|conn| {
        conn.prepare("SELECT id FROM faucets")?
...
async fn read(db: Db, id: i64) -> Option<Json<Post>> {
    let post = db.run(move |conn| {
        conn.query_row("SELECT id, account, amount FROM faucets WHERE id = ?1", params![id],
            |r| Ok(Post { id: Some(r.get(0)?), account: r.get(1)?, amount: r.get(2)? }))
    }).await.ok()?;
...
async fn delete(db: Db, id: i64) -> Result<Option<()>> {
    let affected = db.run(move |conn| {
        conn.execute("DELETE FROM faucets WHERE id = ?1", params![id])
    }).await?;
...
async fn destroy(db: Db) -> Result<()> {
    db.run(move |conn| conn.execute("DELETE FROM faucets", params![])).await?;
