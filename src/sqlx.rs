...
async fn create(mut db: Connection<Db>, post: Json<Post>) -> Result<Created<Json<Post>>> {
    ...
    sqlx::query!("INSERT INTO faucets (account, amount) VALUES (?, ?)", post.account, post.amount)
        .execute(&mut *db)
        .await?;
...
async fn list(mut db: Connection<Db>) -> Result<Json<Vec<i64>>> {
    let ids = sqlx::query!("SELECT id FROM faucets")
        .fetch(&mut *db)
...
async fn read(mut db: Connection<Db>, id: i64) -> Option<Json<Post>> {
    sqlx::query!("SELECT id, account, amount FROM faucets WHERE id = ?", id)
        .fetch_one(&mut *db)
        .map_ok(|r| Json(Post { id: Some(r.id), account: r.account, amount: r.amount }))
        .await
        .ok()
}
...
async fn delete(mut db: Connection<Db>, id: i64) -> Result<Option<()>> {
    let result = sqlx::query!("DELETE FROM faucets WHERE id = ?", id)
        .execute(&mut *db)
        .await?;
...
async fn destroy(mut db: Connection<Db>) -> Result<()> {
    sqlx::query!("DELETE FROM faucets").execute(&mut *db).await?;
